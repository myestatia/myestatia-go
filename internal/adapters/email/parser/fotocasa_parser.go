...
func (p *FotocasaParser) Parse(subject, body string) (*entity.ParsedLead, error) {
	...
	f = func(n *html.Node) {
		if n.Type == html.TextNode {
			...
			// Check for field labels and extract values
			if strings.Contains(text, "Nombre:") {
				extractedData["nameLabel"] = text
			} else if strings.Contains(text, "Teléfono:") {
				extractedData["phoneLabel"] = text
			} else if strings.Contains(text, "Email:") {
				extractedData["emailLabel"] = text
			} else if strings.Contains(text, "Mensaje:") {
				extractedData["messageLabel"] = text
			} else if extractedData["nameLabel"] != "" && extractedData["name"] == "" && text != "" {
				extractedData["name"] = text
				extractedData["nameLabel"] = ""
			} else if extractedData["phoneLabel"] != "" && extractedData["phone"] == "" && text != "" {
				extractedData["phone"] = text
				extractedData["phoneLabel"] = ""
			} else if extractedData["emailLabel"] != "" && extractedData["email"] == "" && text != "" && strings.Contains(text, "@") {
				extractedData["email"] = text
				extractedData["emailLabel"] = ""
			} else if extractedData["messageLabel"] != "" && extractedData["message"] == "" && text != "" && len(text) > 10 {
				extractedData["message"] = text
				extractedData["messageLabel"] = ""
			}
		}
	...
	// Merge data, preferring HTML parsed data
